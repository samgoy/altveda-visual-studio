...
const looksLikeCacheCorruption = (err) => {
    var _a;
    if (!(err instanceof Error)) {
        return false;
    }
    const message = err.message + ((_a = err.stack) !== null && _a !== void 0 ? _a : '');
    return (
...
const internalBundle = async (actualArgs) => {
    ...
    if (actualArgs.rspack) {
        ...
        try {
            ...
            if (looksLikeCacheCorruption(err) && actualArgs.enableCaching !== false) {
                ...
                console.warn('Webpack cache appears to be corrupted. Clearing cache and retrying...');
                await (0, webpack_cache_1.clearCache)(resolvedRemotionRoot, 'production');
                await runWebpack();
            }
            else {
                throw err;
    ...
    const bundleOutput = node_path_1.default.join(outDir, no_react_1.NoReactInternals.bundleName);
    if (!node_fs_1.default.existsSync(bundleOutput)) {
        throw new Error(`The bundler completed without emitting ${no_react_1.NoReactInternals.bundleName}.`);
    }
