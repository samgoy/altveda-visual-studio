...
var parseSrt = ({ input }) => {
  const inputLines = input.replace(/^\uFEFF/, "").replace(/\r\n/g, `
`).replace(/\r/g, `
`).split(`
`);
  const captions = [];
  for (let i = 0;i < inputLines.length; i++) {
    ...
    if (line?.match(/^\s*\d+\s*$/) && nextLine?.includes(" --> ")) {
      ...
      const start = toSeconds(nextLineSplit[0].trim());
      const end = toSeconds(nextLineSplit[1].trim());
      captions.push({
        text: "",
        startMs: start * 1000,
        endMs: end * 1000,
