"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.allApiRoutes = void 0;
const add_effect_1 = require("./routes/add-effect");
const add_effect_keyframe_1 = require("./routes/add-effect-keyframe");
const add_keyframes_1 = require("./routes/add-keyframes");
const add_render_1 = require("./routes/add-render");
const add_sequence_keyframe_1 = require("./routes/add-sequence-keyframe");
const apply_codemod_1 = require("./routes/apply-codemod");
const apply_visual_control_change_1 = require("./routes/apply-visual-control-change");
const batch_update_keyframe_settings_1 = require("./routes/batch-update-keyframe-settings");
const cancel_render_1 = require("./routes/cancel-render");
const composition_component_info_1 = require("./routes/composition-component-info");
const convert_figma_clipboard_to_svg_1 = require("./routes/convert-figma-clipboard-to-svg");
const copy_render_output_to_asset_1 = require("./routes/copy-render-output-to-asset");
const default_coding_agent_1 = require("./routes/default-coding-agent");
const default_editor_1 = require("./routes/default-editor");
const delete_effect_1 = require("./routes/delete-effect");
const delete_jsx_node_1 = require("./routes/delete-jsx-node");
const delete_keyframes_1 = require("./routes/delete-keyframes");
const delete_static_file_1 = require("./routes/delete-static-file");
const download_remote_asset_1 = require("./routes/download-remote-asset");
const duplicate_effect_1 = require("./routes/duplicate-effect");
const duplicate_jsx_node_1 = require("./routes/duplicate-jsx-node");
const find_in_file_1 = require("./routes/find-in-file");
const insert_element_1 = require("./routes/insert-element");
const insert_jsx_element_1 = require("./routes/insert-jsx-element");
const install_remotion_skill_1 = require("./routes/install-remotion-skill");
const invalidate_bundle_1 = require("./routes/invalidate-bundle");
const log_studio_error_1 = require("./routes/log-studio-error");
const move_keyframes_1 = require("./routes/move-keyframes");
const open_in_editor_1 = require("./routes/open-in-editor");
const open_in_file_explorer_1 = require("./routes/open-in-file-explorer");
const open_in_git_client_1 = require("./routes/open-in-git-client");
const open_in_terminal_1 = require("./routes/open-in-terminal");
const paste_effects_1 = require("./routes/paste-effects");
const prepare_element_install_1 = require("./routes/prepare-element-install");
const project_info_1 = require("./routes/project-info");
const redo_1 = require("./routes/redo");
const register_client_render_1 = require("./routes/register-client-render");
const release_notes_1 = require("./routes/release-notes");
const remotion_skills_info_1 = require("./routes/remotion-skills-info");
const remove_render_1 = require("./routes/remove-render");
const rename_static_file_1 = require("./routes/rename-static-file");
const reorder_effect_1 = require("./routes/reorder-effect");
const reorder_sequence_1 = require("./routes/reorder-sequence");
const restart_studio_1 = require("./routes/restart-studio");
const save_effect_props_1 = require("./routes/save-effect-props");
const save_multiple_effect_props_1 = require("./routes/save-multiple-effect-props");
const save_sequence_props_1 = require("./routes/save-sequence-props");
const shutdown_studio_1 = require("./routes/shutdown-studio");
const split_jsx_sequence_1 = require("./routes/split-jsx-sequence");
const split_video_from_audio_1 = require("./routes/split-video-from-audio");
const subscribe_to_default_props_1 = require("./routes/subscribe-to-default-props");
const subscribe_to_file_existence_1 = require("./routes/subscribe-to-file-existence");
const subscribe_to_sequence_props_1 = require("./routes/subscribe-to-sequence-props");
const undo_1 = require("./routes/undo");
const unregister_client_render_1 = require("./routes/unregister-client-render");
const unsubscribe_from_default_props_1 = require("./routes/unsubscribe-from-default-props");
const unsubscribe_from_file_existence_1 = require("./routes/unsubscribe-from-file-existence");
const unsubscribe_from_sequence_props_1 = require("./routes/unsubscribe-from-sequence-props");
const update_available_1 = require("./routes/update-available");
const update_default_props_1 = require("./routes/update-default-props");
const update_effect_keyframe_settings_1 = require("./routes/update-effect-keyframe-settings");
const update_element_install_target_1 = require("./routes/update-element-install-target");
const update_sequence_keyframe_settings_1 = require("./routes/update-sequence-keyframe-settings");
const upgrade_remotion_1 = require("./routes/upgrade-remotion");
exports.allApiRoutes = {
    '/api/invalidate-bundle': invalidate_bundle_1.invalidateBundleHandler,
    '/api/composition-component-info': composition_component_info_1.compositionComponentInfoHandler,
    '/api/copy-render-output-to-asset': copy_render_output_to_asset_1.copyRenderOutputToAssetHandler,
    '/api/convert-figma-clipboard-to-svg': convert_figma_clipboard_to_svg_1.convertFigmaClipboardToSvgHandler,
    '/api/cancel': cancel_render_1.handleCancelRender,
    '/api/render': add_render_1.handleAddRender,
    '/api/unsubscribe-from-file-existence': unsubscribe_from_file_existence_1.unsubscribeFromFileExistence,
    '/api/subscribe-to-file-existence': subscribe_to_file_existence_1.subscribeToFileExistence,
    '/api/remove-render': remove_render_1.handleRemoveRender,
    '/api/find-in-file': find_in_file_1.findInFileHandler,
    '/api/open-in-file-explorer': open_in_file_explorer_1.handleOpenInFileExplorer,
    '/api/open-in-terminal': open_in_terminal_1.openInTerminalHandler,
    '/api/open-in-git-client': open_in_git_client_1.openInGitClientHandler,
    '/api/open-in-editor': open_in_editor_1.openInEditorHandler,
    '/api/open-in-coding-agent': default_coding_agent_1.openInCodingAgentHandler,
    '/api/register-client-render': register_client_render_1.registerClientRenderHandler,
    '/api/unregister-client-render': unregister_client_render_1.unregisterClientRenderHandler,
    '/api/update-default-props': update_default_props_1.updateDefaultPropsHandler,
    '/api/apply-visual-control-change': apply_visual_control_change_1.applyVisualControlHandler,
    '/api/apply-codemod': apply_codemod_1.applyCodemodHandler,
    '/api/subscribe-to-default-props': subscribe_to_default_props_1.subscribeToDefaultProps,
    '/api/unsubscribe-from-default-props': unsubscribe_from_default_props_1.unsubscribeFromDefaultProps,
    '/api/subscribe-to-sequence-props': subscribe_to_sequence_props_1.subscribeToSequenceProps,
    '/api/unsubscribe-from-sequence-props': unsubscribe_from_sequence_props_1.unsubscribeFromSequenceProps,
    '/api/save-sequence-props': save_sequence_props_1.saveSequencePropsHandler,
    '/api/save-effect-props': save_effect_props_1.saveEffectPropsHandler,
    '/api/save-multiple-effect-props': save_multiple_effect_props_1.saveMultipleEffectPropsHandler,
    '/api/add-effect': add_effect_1.addEffectHandler,
    '/api/reorder-effect': reorder_effect_1.reorderEffectHandler,
    '/api/duplicate-effect': duplicate_effect_1.duplicateEffectHandler,
    '/api/reorder-sequence': reorder_sequence_1.reorderSequenceHandler,
    '/api/delete-keyframes': delete_keyframes_1.deleteKeyframesHandler,
    '/api/move-keyframes': move_keyframes_1.moveKeyframesHandler,
    '/api/add-sequence-keyframe': add_sequence_keyframe_1.addSequenceKeyframeHandler,
    '/api/add-effect-keyframe': add_effect_keyframe_1.addEffectKeyframeHandler,
    '/api/add-keyframes': add_keyframes_1.addKeyframesHandler,
    '/api/update-sequence-keyframe-settings': update_sequence_keyframe_settings_1.updateSequenceKeyframeSettingsHandler,
    '/api/update-effect-keyframe-settings': update_effect_keyframe_settings_1.updateEffectKeyframeSettingsHandler,
    '/api/batch-update-keyframe-settings': batch_update_keyframe_settings_1.batchUpdateKeyframeSettingsHandler,
    '/api/delete-effect': delete_effect_1.deleteEffectHandler,
    '/api/paste-effects': paste_effects_1.pasteEffectsHandler,
    '/api/delete-jsx-node': delete_jsx_node_1.deleteJsxNodeHandler,
    '/api/duplicate-jsx-node': duplicate_jsx_node_1.duplicateJsxNodeHandler,
    '/api/split-jsx-sequence': split_jsx_sequence_1.splitJsxSequenceHandler,
    '/api/split-video-from-audio': split_video_from_audio_1.splitVideoFromAudioHandler,
    '/api/update-available': update_available_1.handleUpdate,
    '/api/release-notes': release_notes_1.getReleaseNotesHandler,
    '/api/remotion-skills-info': remotion_skills_info_1.remotionSkillsInfoHandler,
    '/api/install-remotion-skill': install_remotion_skill_1.installRemotionSkillHandler,
    '/api/remove-remotion-skill': install_remotion_skill_1.removeRemotionSkillHandler,
    '/api/project-info': project_info_1.projectInfoHandler,
    '/api/delete-static-file': delete_static_file_1.deleteStaticFileHandler,
    '/api/rename-static-file': rename_static_file_1.renameStaticFileHandler,
    '/api/upgrade-remotion': upgrade_remotion_1.handleUpgradeRemotion,
    '/api/shutdown-studio': shutdown_studio_1.handleShutdownStudio,
    '/api/restart-studio': restart_studio_1.handleRestartStudio,
    '/api/default-coding-agent-info': default_coding_agent_1.getDefaultCodingAgentInfoHandler,
    '/api/default-editor-info': default_editor_1.getDefaultEditorInfoHandler,
    '/api/insert-jsx-element': insert_jsx_element_1.insertJsxElementHandler,
    '/api/insert-element': insert_element_1.insertElementHandler,
    '/api/prepare-element-install': prepare_element_install_1.prepareElementInstallHandler,
    '/api/update-element-install-target': update_element_install_target_1.updateElementInstallTargetHandler,
    '/api/download-remote-asset': download_remote_asset_1.downloadRemoteAssetHandler,
    '/api/undo': undo_1.undoHandler,
    '/api/redo': redo_1.redoHandler,
    '/api/log-studio-error': log_studio_error_1.logStudioErrorHandler,
};
